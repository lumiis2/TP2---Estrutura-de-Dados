...
/**
 * @brief Functor para comparar elementos usando o segundo valor do primeiro par (para A*).
 * 
 * @tparam T O tipo dos elementos a serem comparados.
 */
template <typename T>
struct CompareAstar {
    /**
     * @brief Operador de comparação para comparação de menor que com base no segundo valor do primeiro par.
     * 
     * @param lhs Operando do lado esquerdo.
     * @param rhs Operando do lado direito.
     * @return true se o segundo valor de lhs.first for menor que o segundo valor de rhs.first.
     * @return false caso contrário.
     */
    bool operator()(const T& lhs, const T& rhs) const {
